...
const COPY = {
  de: {
    online: "Online — probier es aus, es ist live",
    tag: "Live-Demo",
    note: "Das ist das echte Produkt — dieselben KI-Mitarbeiter, die du im Dashboard einrichtest.",
  },
  en: {
    online: "Online — try it, it's live",
    tag: "Live demo",
    note: "This is the real product — the same AI employees you configure in the dashboard. Demo replies are in German; with AI keys configured, agents answer in your language.",
  },
} as const;

/** Landing-Demo: einen der 4 KI-Mitarbeiter auswählen und live chatten. */
export function LiveDemo({ locale = "de" }: { locale?: Locale }) {
      ...
      <div className="glass overflow-hidden rounded-2xl border border-border/80 shadow-2xl shadow-black/40">
        ...
        <div className="h-[420px] sm:h-[460px]">
          <AgentChat
            ...
            greeting={locale === "en" ? template.demoGreetingEn : template.demoGreeting}
            suggestions={
              locale === "en" ? [...template.suggestedQuestionsEn] : [...template.suggestedQuestions]
            }
            compact
          />
        </div>
